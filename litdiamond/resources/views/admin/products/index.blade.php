...
<html>
    <head>
        ...
        <title>LIT Diamond | Products</title>
        ...
        <!-- DataTables -->
        <link rel="stylesheet" href="{{ url('resources/admin') }}/bower_components/datatables.net-bs/css/dataTables.bootstrap.min.css">
        <!-- Theme style -->
    ...
    <body class="hold-transition skin-blue sidebar-mini">
        <div class="wrapper">
            ...
            <div class="content-wrapper">
                ...
                <!-- Main content -->
                {{ Form::open(['url' => 'admin/products', 'id' => 'myForm']) }}
                            ...
                            <div class="box">
                                <div class="box-header">
                                    ...
                                    <div class="pull-right">
                                        <button class="btn btn-primary btn-xs" type="button" onclick="location.href='{{ url('admin/products/add') }}'">Add New</button>
                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            foreach ($Products as $product) {
                                                ?>
                                                <tr>
                                                    <td><input type="checkbox" class="chk" name="IDs[]" value="{{ $product->ProductID }}" /></td>
                                                    <td>{{ $product->ProductID }}</td>
                                                    <td>{{ $product->CategoryName }}</td>
                                                    <td>{{ $product->ProductName }}</td>
                                                    <td><img src="{{ url('public/uploads/products/'.$product->Image) }}" width="100" /></td>
                                                    <td>{{ $Statuses[$product->Status ] }}</td>
                                                    <td>{!! $product->DateAdded !!}</td>
                                                    <td>{!! $product->DateModified !!}</td>
                                                    <td><button type="button" class="btn btn-primary btn-xs" onclick="location.href='{{ url('admin/products/edit/'.$product->ProductID) }}'">Edit</button></td>
                                                </tr>
                                                <?php
                                            }
                                            ?>
        ...
        <!-- DataTables -->
        <script src="{{ url('resources/admin') }}/bower_components/datatables.net/js/jquery.dataTables.min.js"></script>
        <script src="{{ url('resources/admin') }}/bower_components/datatables.net-bs/js/dataTables.bootstrap.min.js"></script>
        <!-- SlimScroll -->
